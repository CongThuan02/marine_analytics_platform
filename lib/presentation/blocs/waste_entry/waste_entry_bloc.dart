...
class WasteEntryBloc extends Bloc<WasteEntryEvent, WasteEntryState> {
  ...
  Future<void> _onLoad(LoadWasteEntries event, Emitter<WasteEntryState> emit) async {
    emit(state.copyWith(status: Status.loading, clearMessage: true));
    try {
      final entries = await _repository.fetchWasteEntries();
      emit(state.copyWith(status: Status.loaded, entries: entries, clearMessage: true));
    } catch (e) {
      emit(state.copyWith(status: Status.fail, message: e.toString()));
    }
  }

  Future<void> _onCreate(CreateWasteEntry event, Emitter<WasteEntryState> emit) async {
    emit(state.copyWith(status: Status.loading, clearMessage: true));
    try {
      final message = await _repository.createWasteEntry(entry: event.entry);
      final entries = await _repository.fetchWasteEntries();
      emit(state.copyWith(status: Status.success, message: message, entries: entries));
    } catch (e) {
      emit(state.copyWith(status: Status.fail, message: e.toString()));
    }
  }
