...
class ReminderBloc extends Bloc<ReminderEvent, ReminderState> {
  ...
  Future<void> _onLoadReminders(
    ...
    result.fold((failure) => emit(ReminderError(failure.message)), (reminders) {
      ...
      ReminderNotificationService().rescheduleAllReminders(reminders);
    });
  }
  ...
  Future<void> _onCreateReminder(
    ...
    result.fold((failure) => emit(ReminderError(failure.message)), (reminder) {
      // Schedule notification for new reminder
      ReminderNotificationService().scheduleReminder(reminder);
  ...
  Future<void> _onToggleReminder(
    ...
    result.fold((failure) => emit(ReminderError(failure.message)), (_) {
      // Cancel or reschedule notification based on enabled status
      if (!event.enabled) {
        ReminderNotificationService().cancelReminder(event.id);
      }
      add(const LoadReminders());
    });
  }
